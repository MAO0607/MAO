...
from utils import get_easyDict_from_filepath,cosine_similarity,log_and_print_online, now
...
"""
for overall memory, related to MemoryCards.json
"""
...
class AllMemory(MemoryBase):
    ...
    def search_text(self, code_query, k:int):
        ...
        if self._get_memory_count() == 0 or code_query == None or k == 0:
            ...
            # print(code_embedding_data.shape)
            # print(code_query.shape)

            # gurantee the similarities correct
            # temp = [cosine_similarity(data,code_query.reshape(-1,1)) for data in code_embedding_data] 
            ...
            # print(filtered_similarities, sourceMIDS, task_list, task_dir_list)
    ...
    def search_code(self, text_query, k:int):
        ...
        if self._get_memory_count() == 0 or text_query == None or k == 0:
            ...
            # gurantee the similarities correct
            # temp = [cosine_similarity(data,text_query.reshape(-1,1)) for data in text_embedding_data] 
            ...
            # print(filtered_similarities,targetMIDs,task_list, task_dir_list)
            return target_code, filtered_similarities, targetMIDs, task_list, task_dir_list


"""
 This class is currently unused but is being kept for future extensions.
"""
...
class UserMemory(MemoryBase):
    ...
    def memory_retrieval(self,
        ...
        return "test memory here "
    def memory_update() -> None:
        pass
"""
 This class is currently unused but is being kept for future extensions.
"""
class AssistantMemory(MemoryBase):
    def __init__(self, directory: str):
        super().__init__(directory)
...
"""
high level class for all memory accesses
"""
class Memory:
    def __init__(self):
        ...
        self.update_count = 0
        self.memory_keys: List[str] = ["All","CodeReviewer","Programmer","SoftwareTestEngineer"]
        self.memory_data = {}


    def __str__(self) -> str:
        if self.memory_data.get("All") == None:
            return "current memory not loaded"
        else:
    ...
    # create memory path and upload memory from local json             
    def upload(self,str=""):
        self.directory = os.path.join(os.getcwd(),"mmm","memory")
        if os.path.exists(self.directory) is False:
            os.mkdir(self.directory)
        for key in self.memory_keys:
            if key =="All":
                if str!="":
                    filename = "MemoryCards_"+str+".json"
                ...
                path = os.path.join(self.directory,key+"_Memory.json")
                self.memory_data[key] = AssistantMemory(path)
    ...
    def upload_from_experience(self, experience):
        ...
        with open(self.memory_data["All"].directory, 'w') as file:
            ...
            if self.memory_data["All"].content != None and len(self.memory_data["All"].content)!=0:
                #if combined_json_str["dir"] not in dirList:
