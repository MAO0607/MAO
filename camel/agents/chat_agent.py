...
class ChatAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.model: ModelType = (model if model is not None else ModelType.GPT_4)
        ...
        if memory !=None and self.role_name in["Code Reviewer","Programmer","Software Test Engineer"]:
            #self.memory = memory.memory_data.get(self.role_name.strip().replace(" ",""))
            ...
            if self.role_name == "Programmer":
                ...
                if result != None:
                    ...
                    if target_memory != None and len(target_memory) != 0:
                        ...
                        log_and_print_online(self.role_name,
    ...
    def step(
        ...
        # use the memory and add a system messgae in the chat
        #messages = self.use_memory(input_message)
